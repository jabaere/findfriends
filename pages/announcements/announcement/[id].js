...
import { AUTH_KEY } from '../../../utils/Constants';
 ...
 const EditAnnouncement = ({statement}) => {
  ...
  const {AlertUser,alert,getAnnouncementId} = useContext(AuthContext)
  ...
  const onSubmit = async (e) => {
   ...
   //console.log(ddt)
    ...
    const response = await fetch(`http://localhost:1337/api/announcements/${statement.data.id}`, {
      method: 'PUT',
      mode: 'cors',
  ...
  return (
    ...
    <Container maxWidth="xs">
      ...
      <Box mb={2}>
        ...
        <Box mb={2}>
            <ImageUpload 
               announcementId={statement.data.id} 

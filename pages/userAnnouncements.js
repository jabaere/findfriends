...
import AuthContext from '../context/AuthContext';
 ...
 //item.attributes.users_permissions_user.data.attributes.email
      ...
      if(user){
        const count = announcements.data.filter(item=> item.attributes.email !== null)
  ...
  return (
    <div className={styles.cardsContainer} id="dasda">
      ...
      <Box sx={{width: 400, display:'flex', justifyContent:'flex-start', padding: 1}}>
      ...
      {user && announcements.data.filter(item=> item.attributes.email !== null)
      .filter(item => item.attributes.email===user.email)
      .map(announcement=> (
